...
export function ShopImportForm() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setResult(r);
      router.refresh();
      const { inserted, updated, skipped, errors } = r.summary;
      const sheetLabel = r.sheetUsed ? ` (シート: ${r.sheetUsed})` : "";
      if (errors > 0) {
        setToast({ kind: "warn", msg: `取込完了${sheetLabel}: 新規 ${inserted} / 更新 ${updated} 件 (エラー ${errors} 件)` });
      } else {
        setToast({ kind: "ok", msg: `取込成功!${sheetLabel} 新規 ${inserted} / 更新 ${updated} / スキップ ${skipped}` });
      }
    } catch (e) {
    ...
    <>
      {toast && (
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <button type="button" onClick={() => setToast(null)} className="ml-2 opacity-70 hover:opacity-100" aria-label="閉じる">✕</button>
          </div>
        </div>
      )}
      ...
      <div className="card p-5 space-y-3">
        <input
          ...
          className="text-sm"
        />
        <button type="button" className="btn-primary" disabled={busy || !file} onClick={handleUpload}>
          {busy ? "取込中…" : "インポート"}
        </button>
        {error && <div className="text-sm text-rose-700 bg-rose-50 p-3 rounded">{error}</div>}
      </div>

      {result && (
        <div className="card p-5 space-y-4 mt-4">
          {result.sheetUsed && (
            <p className="text-xs text-slate-500">取込元シート: <code className="font-mono">{result.sheetUsed}</code></p>
          )}
          ...
          {result.inserted.length > 0 && (
            <Section title={`✓ 新規追加 (${result.inserted.length} 件)`} initiallyOpen>
              <ul className="text-sm space-y-1">
                {result.inserted.map((t, i) => <li key={i} className="text-emerald-700">+ {t}</li>)}
              </ul>
            </Section>
          )}

          {result.updated.length > 0 && (
            <Section title={`↻ 更新 (${result.updated.length} 件 / email 一致で上書き)`} initiallyOpen>
          ...
          {result.errors.length > 0 && (
            <Section title={`✕ エラー (${result.errors.length} 件)`} initiallyOpen>
              <ul className="text-sm space-y-1">
                {result.errors.map((e, i) => <li key={i} className="text-rose-700">{e}</li>)}
              </ul>
            </Section>
          )}
        </div>
...
function StatCard({ label, value, tone }: { label: string; value: number; tone: "ok" | "info" | "neutral" | "err" }) {
  const toneClass =
...
function Section({ title, children, initiallyOpen }: { title: string; children: React.ReactNode; initiallyOpen?: boolean }) {
  return (
    <details open={initiallyOpen} className="border border-slate-200 rounded">
      <summary className="px-3 py-2 text-sm font-semibold cursor-pointer bg-slate-50 hover:bg-slate-100 rounded">{title}</summary>
