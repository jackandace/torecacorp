...
export function GachaPlay({boxId,cost,name,resultHref="/gacha/result"}:{boxId:number;cost:number;name:string;resultHref?:string}) {
 ...
 useEffect(()=>{if(!playing)return;const timer=setTimeout(()=>window.location.assign(sitePath(resultHref)),3200);return()=>clearTimeout(timer);},[playing,resultHref]);
 ...
 {playing?<div className="gacha-animation"><img src={sitePath("/assets/parade-puff.png")} alt=""/><strong>どんなお宝に出会えるかな？</strong><span>{count}連ガチャ</span><button className="button outline-button" onClick={()=>window.location.assign(sitePath(resultHref))}>演出をスキップ</button></div>:<>
 ...
 {short?<div className="insufficient-coins"><p>あと {(needed-wallet.balance).toLocaleString()} コイン必要です</p><a className="button primary-button" href={sitePath("/point")}>コインをチャージ</a></div>:<button className="button primary-button" disabled={!wallet.ready} onClick={run}>{count==='1'?'1回':count+'連'}ガチャを実行</button>}
